...
class CSPModifier:
    ...
    def responseheaders(self, flow):
        ...
        try:
            ...
            if not csp_headers:
                ...
                if VERBOSE_LOGGING:
                    ctx.log.debug(f"[CSP] Generated nonce for {flow.request.host}")
                return
            ...
            if VERBOSE_LOGGING:
                ctx.log.debug(f"[CSP] Nonce ready: {nonce[:8]}...")
    ...
    def response(self, flow):
        ...
        try:
            ...
            if not nonce:
                ctx.log.warn(f"[CSP] Missing nonce for {flow.request.host}, skipping")
                return

            script_hashes = flow.metadata.get('script_hashes', [])
            if not script_hashes:
                ctx.log.warn(f"[CSP] No script hashes from injector - possible hook order issue")
            ...
            for header_name, original_csp in csp_headers:
                ...
                if new_csp != original_csp:
                    ...
                    modified = True
                    if VERBOSE_LOGGING:
                        ctx.log.debug(f"[CSP] Modified {header_name} (+{len(script_hashes)} hashes)")
